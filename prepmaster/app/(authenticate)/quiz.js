    ...
    <SafeAreaView style={styles.container}>
        ...
        {question && question.length > 0 ? (
            ...
            <View style={{alignSelf:'center'}}>
              <Pressable style={styles.options} disabled={selected} onPress={() => handleCheck(question[currquestion].op1)}><Text>{question[currquestion].op1}</Text></Pressable>
              <Pressable style={styles.options} disabled={selected} onPress={() => handleCheck(question[currquestion].op2)}><Text>{question[currquestion].op2}</Text></Pressable>
              <Pressable style={styles.options} disabled={selected} onPress={() => handleCheck(question[currquestion].op3)}><Text>{question[currquestion].op3}</Text></Pressable>
              <Pressable style={styles.options} disabled={selected} onPress={() => handleCheck(question[currquestion].op4)}><Text>{question[currquestion].op4}</Text></Pressable>
            </View>
            </>
          ) : (<ActivityIndicator size={'large'} />)}
...
const styles = StyleSheet.create({
  ...
  options: {
    backgroundColor:"white", borderRadius:5, marginLeft:"auto", marginRight:"auto", padding:10, borderWidth:1
  }
})